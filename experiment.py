...
class Experiment(object):
    ...
    def run(self) -> None:
        if not self.manual_output:
            subprocess.call(["python3" if sys.platform == "linux" else "python", self.py_file],
                            stdout=open(self.output_file, 'w'))
        else:
            open(self.output_file, 'w').write(
                open(self.manual_output, 'r').read())
    ...
    def create_docx(self) -> None:
        ...
        path_to_template = os.path.join(
            os.path.dirname(current_file), 'template.docx')
        ...
        r.font.name = 'Times New Roman'
        r.underline = True
        doc.add_picture(self.py_file.replace(
            '.py', '_script.jpg'), width=shared.Cm(17))
        p = doc.add_paragraph('')
        ...
        r.underline = True
        doc.add_picture(self.py_file.replace(
            '.py', '_out.jpg'), width=shared.Cm(17))
